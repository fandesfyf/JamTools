# from jam_files import *
# 2.4更新：帧提取和图片裁剪功能会将输出放到与文件同名的文件夹中提示直接打开；增加批量重命名功能、图片转视频功能、视频转码中增加设置帧率、录屏默认使用H.264的高画质提高压缩比，可能导致某些低端设备无法播放，请勾选画质适配！字体更换wryh
# 修复设置写入注册表失败的问题，\酱控制加入播放速度调整最大支持100倍(速度太快时可能拖拽失效)加入安全开关、键盘鼠标动作将严格同步，支持关闭所有通知,音视频混合自动裁剪音频长度,视频速度调整、支持无损转换
# 所有倒计时（截屏、录屏、控制）过程不在卡顿！倒计时可停止,酱翻译中英文自动切换，酱转换后台线程处理，界面不卡顿，按钮颜色随处理状态改变,多任务处理控制
# 录屏gif转换优化,处理过程可中止,所有控件加入提示
import hashlib
import json
import socket
import gc
...
class Swindow(QMainWindow):
    ...
    def help(self):
        ...
        text = """>>>功能简介:
1.酱截屏：快捷键Alt+z；点击后选区截图，点复制键复制到剪切板；截图过程中右键返回、支持截图中文字识别(翻译)、图像识别等，左侧工具栏提供画笔橡皮擦等；支持滚动截屏，滚动过程中鼠标单击或画面停止将中止截屏，进行图片拼接并复制。

2.酱识字：快捷键Alt+x；截屏提取：截屏并提取文字；批量识别：可上传一张或多张图片进行文字提取

3.酱翻译：无快捷键(极简模式下可通过系统托盘使用)；输入输出文本框均可编辑，可手动输入文字翻译，支持多种语言互译！已集成到极简模式、截屏等界面下。

4.酱录屏：快捷键Alt+c;屏幕录制功能，支持gif等多种格式录制；可以选定录制区，不选则为全屏录制；支持自定义编码速率、帧率、视频质量、声音源鼠标等；录屏结束后点击弹出的系统提示将直接播放！
...
7.酱传输:提供快速的局域网传输功能,有客户端点对点连接传输和网页端共享两种方式。均支持数据双向传输。客户端连接需要通过连接码自动搜索并了解主机，建立连接之后即可互相发送文件或文件夹。网页端传输相当于共享文件夹，支持共享一个文件夹或文件夹下的某几个文件，通过选择对应网络适配器即可生成共享链接，连入该与适配器同一网络的其他设备即可通过链接或扫码访问共享文件，网页端勾选允许上传后支持文件上传。
...
剪贴板翻译：监控剪贴板内容，如果检测为英语则自动翻译并显示！可在设置中心设置详细内容！
...
视频播放器：内置视频播放器功能，支持文件拖入自动添加播放列表！
##所有功能均可以在系统托盘调用！
##建议尽量用快捷键！（方便快捷）
留意软件内状态栏和悬浮提示。。。尽情使用吧！
hhh(o゜▽゜)o☆）
